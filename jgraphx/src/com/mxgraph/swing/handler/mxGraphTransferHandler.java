...
public class mxGraphTransferHandler extends TransferHandler
{
	...
	public mxGraphTransferable createGraphTransferable(
			mxGraphComponent graphComponent, Object[] originalCells2, ImageIcon icon)
	{
		mxGraph graph = graphComponent.getGraph();
		mxPoint tr = graph.getView().getTranslate();
		double scale = graph.getView().getScale();

		mxRectangle bounds = graph.getPaintBounds(originalCells2);
		...
		return createGraphTransferable(graphComponent, originalCells2, bounds, icon);
	}
			...
			Object[] originalCells2)
	{
		...
		Image img = mxCellRenderer.createBufferedImage(
				graphComponent.getGraph(), originalCells2, 1, bg,
