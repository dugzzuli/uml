...
public class mxGraphHandler extends mxMouseAdapter implements
		DropTargetListener
{
	private String cellId;//获取图元绘制时图元的id
	...
	/**
	 * Holds the start location of the mouse gesture.鼠标手势
	 */
	...
	protected void installDragGestureHandler()
	{
									...
									new Point(), t, new DragSourceAdapter()
									{
										public void dragDropEnd(DragSourceDropEvent dsde)
										{
											((mxGraphTransferHandler) th)
													.exportDone(graphComponent,t,//在导出数据后调用
		...
		DragSource dragSource = new DragSource();//创建拖拽源
		dragSource.createDefaultDragGestureRecognizer(
				graphComponent.getGraphControl(),
				DnDConstants.ACTION_COPY_OR_MOVE, dragGestureListener);//建立拖拽源和事件的联系
	}
	...
	public void drop(DropTargetDropEvent e)
	{
		if (canImport)
		{
			...
			try
			{
				Transferable t = handler.lastImported;
				...
				if (t.isDataFlavorSupported(mxGraphTransferable.dataFlavor))
				{
					...
					//拖入不同图元显示不同的属性界面
					mxCell[] mxcells = (mxCell[])dragCells;
				}
				}catch (Exception ex)
				{
					// do nothing
