...
public class mxGraph extends mxEventSource
{
	...
	/**
	 * Property change event handling.支持特性改变
	 */
	protected PropertyChangeSupport changeSupport = new PropertyChangeSupport(
			this);

	/**
	 * Holds the model that contains the cells to be displayed.用于展示cells的model
	 */
	protected mxIGraphModel model;

	/**
	 * Holds the view that caches the cell states.cell属性对应的view
	 */
	protected mxGraphView view;

	/**
	 * Holds the stylesheet that defines the appearance of the cells.定义cells外观的样式表
	 */
	protected mxStylesheet stylesheet;

	/**
	 * Holds the <mxGraphSelection> that models the current selection.当前选定model
	 */
	protected mxGraphSelectionModel selectionModel;

	/**
	 * Specifies the grid size. Default is 10.指定网格尺寸=10
	 */
	protected int gridSize = 10;

	/**
	 * Specifies if the grid is enabled. Default is true.指定网格是否可激活，默认为真
	 */
	 ...
	 * graph改变后容器调整大小时在右下角要加入的border的大小
	 */
	...
	public void swapBounds(Object cell, boolean willCollapse)
	{
		...
		if (cell != null)
		{
			...
			if(mxcell.getFlage() == false){
				mxcell.setFlage(true);
			}else{
				mxcell.setFlage(false);
			}
		}
		
	}
	public boolean isSwapBounds(Object cell)
	{
		mxCell mxcell = (mxCell) cell;
		return mxcell.getFlage();
	}
